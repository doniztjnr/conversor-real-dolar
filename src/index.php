 ...
 *
 * @param string $real
 * @param Label $dolarLabel
 * @return void if incorrect value return nothing
 *
 * Set to label how much dolar is an amount of real
 */
function realToDolar(string $real, Label $dolarLabel): void
{
    ...
    $dolarLabel->text = '$' . (string)round(((float)$real / (float)(new Moeda())->USDBRL()['high']), 2);
}
...
$labelReal = new Label($labelFrameInfo, 'Real - R$' . (new Moeda())->USDBRL()['high']);
$labelReal->font = APP_FONT;
$labelFrameInfo->pack([$labelDolar, $labelReal], ['padx' => 5, 'pady' => 5, 'anchor' => 'w']);
...
$labelFrameConverter->pack([$realEntry, $dolarLabel], ['padx' => 5, 'pady' => 5, 'ipadx' => 3, 'ipady' => 3]);
...
$buttonFrame->pack($converterButton, ['padx' => 5, 'pady' => 5, 'ipadx' => 3, 'ipady' => 3, 'anchor' => 'e']);

$mainWindow->pack([$labelFrameInfo, $labelFrameConverter, $buttonFrame], ['padx' => 5, 'pady' => 2, 'fill' => Pack::FILL_X]);
$app->run();
